...
function Particle() {
  ...
  /*确保粒子速度不为0*/
  while (
    Math.abs(this.speedX) < limitSpeed ||
    Math.abs(this.speedY) < limitSpeed
  ) {
...
function galaxyPlot() {
  ...
  if (gfocus && tmp_angle != angle) {
    var xrate = Math.floor((100 * mouseX) / header.scrollWidth),
      yrate = Math.floor(
        (100 * (mouseY + document.documentElement.scrollTop)) /
          header.scrollHeight
      );
  ...
  for (let i = count - 1; -1 < i; i--) {
    ...
    if (
      gfocus &&
      62500 >= sqr(particles[i].x, mouseX, particles[i].y, mouseY)
    ) {
      galaxyCtx.beginPath();
    ...
    for (let j = count - 1; i != j && -1 < j; j--) {
      ...
      if (
        10000 >=
        sqr(particles[i].x, particles[j].x, particles[i].y, particles[j].y)
      ) {
...
let main = (timestamp, elapsed) => {
  ...
  window.requestAnimationFrame((_timestamp) =>
    main(_timestamp, elapsed + _timestamp - timestamp)
  );
};
window.requestAnimationFrame((timestamp) => main(timestamp, 0));
